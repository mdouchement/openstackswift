...
func TestDownloadObject(t *testing.T) {
	...
	payload, err := c.ObjectGetString(ctx, "Xcontainer", "a1/b2/c3.txt")
	assert.NoError(t, err)
	assert.Equal(t, content, payload)
}

func TestDownloadLargeObject(t *testing.T) {
	c, cleanup := setup()
	defer cleanup()
	...
	oof, _, err := c.ObjectOpen(ctx, "Xcontainer", "a42/dates.txt", true, swift.Headers{})
	assert.NoError(t, err)
	defer oof.Close()
